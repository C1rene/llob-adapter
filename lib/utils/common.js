...
let { logbotinfo, replaceText } = cfg()
...
export default new class nccommon {
    ...
    async format(msg, message_id, isNode = false) {
        ...
        for (let i of icmsg) {
            switch (i.type) {
                case 'text':
                    /** 替换文本 */
                    for(let rule of replaceText) {
                        if(rule.key) {
                            if(rule.key.startsWith('/') && rule.key.endsWith('/') && rule.key.length > 1) {
                                const regexBody = rule.key.slice(1, -1)
                                i.text = i.text.replaceAll(new RegExp(regexBody), rule?.value?.trim() || '')
                            } else {
                                i.text = i.text.replaceAll(new RegExp(rule.key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')), rule?.value?.trim() || '')
                            }
                        }
                    }
    ...
    async source(i, group_id, napcat) {
        ...
        try {
            ...
            let { message: parsedMessage, raw_message } = await this.getMessage(source.message, group_id, false)

            source = {
                ...source,
                time: source.message_id,
                seq: source.message_seq || source.message_id,
